...
---------------------------------------------------------- Config ----------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------
local command = true -- false = /smutsig funkar inte, true /smutsig funkar
...
local duschar = { 
    ["Dusch1"] = { ["x"] = -1382.77, ["y"] = -1328.39, ["z"] = 3.3, ["h"] = 261.38}, 
    ["Dusch2"] = { ["x"] = -1243.76, ["y"] = -1537.76, ["z"] = 3.45, ["h"] = 260.36},
	["Lägenhetsdusch1"] = { ["x"] = 254.39, ["y"] = -1000.25, ["z"] = -99.78, ["h"] = 177.36}
} 
local blips = { -- blips på mappen, för allmänna duschar
    {title="Dusch", colour=38, id=355, x = -1382.77, y = -1328.39, z = 4.0},
	{title="Dusch", colour=38, id=355, x = -1243.76, y = -1537.76, z = 4.0}
}
----------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------- Status ----------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		...
		if isSmutsig then
			...
			if TreDMe then
				TriggerServerEvent('3dme:shareDisplay', '~r~Behöver duscha')
			end
			Wait(500)
		end
	end
		...
		for k, v in pairs(duschar) do
			if GetDistanceBetweenCoords(coords, v["x"], v["y"], v["z"], true) < 100 then
				...
				if GetDistanceBetweenCoords(coords, v["x"], v["y"], v["z"], true) < 1.5 then
					DrawText3D(v["x"], v["y"], v["z"]+0.9, '~g~[E] ~w~Duscha', 0.80)
	...
	if command then
		if scriptStatus then
			...
			TriggerEvent("pNotify:SendNotification",{
				text = ('Alla i din närhet känner nu hur illa du luktar!'),
				type = "warning",
				timeout = (5000),
				layout = "bottomCenter",
				queue = "global"
		...
		TriggerEvent("pNotify:SendNotification",{
			text = ('Du kan inte göra dig själv smutsig via kommando!'),
			type = "error",
			timeout = (5000),
			layout = "bottomCenter",
			queue = "global"
